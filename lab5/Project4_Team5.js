...
custom_points.geometry.setAttribute(
  // Custom point의 BufferGeometry정보.
  "position",
...
const cannon_tick = (
  bone // Apply physics // 함수가 호출되면 중력에 의해 sphereBody가 떨어짐
) => {
...
function computeR(A, B) {
  // 회전 행렬 계산해주는 함수. 우리는 짐볼락 현상 방지를 위해 회전 쿼터니언 계산하는 함수를 따로 만들었음.
  // get unit vectors
...
function computeQuaternion(A, B) {
  // 짐볼락 현상 방지를 위해 두 벡터 간의 쿼터니언을 계산하는 함수를 만들었음.
  const uA = A.clone().normalize();
  const uB = B.clone().normalize();
  ...
  if (results.poseLandmarks) {
    ...
    // left arm
    {
      ...
      if (results.leftHandLandmarks) {
        ...
        const SK_u = new THREE.Vector3()
          .subVectors(
            skeleton.getBoneByName("左小指１").position,
            skeleton.getBoneByName("左親指１").position
          ) // 엄지->소지 벡터
          .normalize();
        const SK_v = skeleton
          .getBoneByName("左中指１")
          .clone()
          .position.normalize(); // 손목->중지 벡터
        const SK_w = new THREE.Vector3()
          .crossVectors(
            // 손바닥의 법선 벡터
            SK_u,
            SK_v
          ...
          .multiplyScalar(-1);
        const SK_new_u = new THREE.Vector3().crossVectors(SK_v, SK_w);
        const SK_R = new THREE.Matrix4().makeBasis(SK_new_u, SK_v, SK_w);

        const R_BonetoMP = MP_R.clone().multiply(
          // Detection상 손의 rotation을 LeftHandBone의 rotation으로 변환하는 행렬
    ...
    // right arm
    {
      ...
      if (results.rightHandLandmarks) {
        ...
        let jointRightMiddle1 =
          total_pos_3d_landmarks["RIGHT_MIDDLE_FINGER_MCP"];
        ...
        const SK_u = new THREE.Vector3()
          .subVectors(
            skeleton.getBoneByName("右小指１").position,
            skeleton.getBoneByName("右親指１").position
          ) // 엄지->소지 벡터
          .normalize();
        const SK_v = skeleton
          .getBoneByName("右中指１")
        ...
        const R_BonetoMP = MP_R.clone().multiply(
          // Detection상 손의 rotation을 RightHandBone의 rotation으로 변환하는 행렬
          SK_R.clone().transpose()
        );
        const R_Tpose = R_chain.clone().transpose();
